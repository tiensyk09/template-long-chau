...
export async function POST(request, { params }) {
  ...
  if (!user) {
    return sendResponse(
      { code: 'rest_cannot_create', message: 'Xin lỗi, bạn không có quyền thực hiện hành động này.', data: { status: 401 } },
      401
    );
  }
    ...
    try {
      ...
      if (!fileUrl) {
        try {
          ...
          return sendResponse({ code: 'rest_upload_fs_error', message: fsErr.message, data: { status: 500 } }, 500);
        }
      }
...
export async function DELETE(request, { params }) {
  ...
  if (!user) {
    return sendResponse(
      { code: 'rest_cannot_delete', message: 'Xin lỗi, bạn không có quyền thực hiện hành động này.', data: { status: 401 } },
      401
    );
  }
