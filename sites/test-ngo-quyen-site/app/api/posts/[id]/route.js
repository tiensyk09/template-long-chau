...
export async function PUT(request, { params }) {
  ...
  try {
    ...
    const { title, summary, content, category_id, category_name, image, author, status, featured } = body;
    ...
    await query(
      `UPDATE posts SET
        ...
        image=?, author=?, status=?, featured=?, updated_at=?
       WHERE id=?`,
      [title, summary || '', content || '', category_id || null, category_name || '',
       image || '', author || '', status || 'draft', featured ? 1 : 0, nowStr, id]
    );
