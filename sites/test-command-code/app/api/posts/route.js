...
export async function GET(request) {
  try {
    ...
    let sql = 'SELECT id, slug, title, summary, content, status, views, author_name, created_at FROM posts WHERE 1=1';
    const params = [];
...
export async function POST(request) {
  ...
  try {
    ...
    const result = await query(
      `INSERT INTO posts (slug, title, summary, content, status, author_id, author_name)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [slug, title, summary || '', content || '', status || 'draft', user.id, user.displayName]
    );
