...
function HeroBlock({ configs = {} }) {
  ...
  return (
    <section className="hero" style={{ padding: '60px 0 40px' }}>
      ...
      <h1>{configs.title || 'DAILY KOREAN NEWS'}</h1>
      ...
      <div className="comparison-grid">
        ...
        <div className="console-box with-taste">
          <div className="console-header">
            ...
            <div className="console-title">{configs.rightTitle || 'korean-news.log'}</div>
  ...
  const handleSignupSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        setSignupStatus({ type: 'success', message: '구독 신청이 완료되었습니다! DAILY KOREAN NEWS에 오신 것을 환영합니다.' });
        setEmail('');
      } else {
