...
export async function generateMetadata({ params }) {
  ...
  if (!decoded) return { title: 'Asset Not Found | DAILY KOREAN NEWS' };
  ...
  if (!file || file.is_public === 0) {
    return { title: 'Asset Restricted | DAILY KOREAN NEWS' };
  }

  return {
    title: `Download: ${file.name} | DAILY KOREAN NEWS`,
    description: file.description || `Download asset ${file.name} securely from DAILY KOREAN NEWS.`
  };
}
